...
void RestoreMenuStateBeforeSleep(void); //恢复睡眠菜单
...
void SideKey_Callback(void)
  {
  unsigned char time;
	bool Result,DelayedResult;
	//进行去抖判断
	Result=GPIO_ReadInBit(ExtKey_IOG,ExtKey_IOP)==SET?true:false; //存储15毫秒前的状态
	delay_ms(20); //延时20mS去抖
	DelayedResult=GPIO_ReadInBit(ExtKey_IOG,ExtKey_IOP)==SET?true:false;
	if(DelayedResult!=Result)return;//20毫秒后再度采样状态，如果两次不一致则说明为抖动，此时不进行处理
	//复位睡眠定时器
	RestoreMenuStateBeforeSleep();//恢复睡眠之前的状态
	if(SleepTimer==0)
	  { 
	  SleepTimer=OLEDSleepTimeOut*8;
		return; //如果按键处于睡眠状态则直接进行定时器复位并跳出本次处理
		}
	else SleepTimer=OLEDSleepTimeOut*8; //只进行复位
	//进行按键处理
